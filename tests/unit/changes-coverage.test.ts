...
import {
  ...
  saveFloor, addFloorTable, removeFloorTable,
...
const CALLS: Record<string, () => void | Promise<void>> = {
  ...
  // card on `/admin` changes the moment it lands.
  ...
  // *Ukloni sto* — the same table carried back in. Both entities again, for
  // the same reason as the add.
  [join('tables', '[id]', 'index.delete.ts')]: () => {
    const { id } = addFloorTable(f.db, f.venueId, f.adminActor(), { zone: 'basta', x: 30, y: 30 })
    removeFloorTable(f.db, f.venueId, f.adminActor(), id)
  },
