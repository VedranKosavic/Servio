...
describe('every edit reaches every screen and leaves a record', () => {
  ...
  it('posts nothing to chat — Razgovor is gone, the poll is how phones learn', () => {
    const amar = add('Amar', PON)
    add('Lejla', PON)
    removeFromPattern(f.db, f.venueId, f.adminActor(), amar.id, f.clock.now())
    expect(f.db.select().from(schema.chatMessages).all()).toHaveLength(0)
    expect(f.db.select().from(schema.changes).all().some(c => String(c.entity) === 'chat')).toBe(false)
  })
})
