...
// WP5's `log`/`bump`/`queueAlert` and WP1's `verifyPinMetered` /
// `assertNoPendingOutbox` are real now that both packages have merged, and WP2
// calls them exactly as the contract says. They stay doubled here on purpose:
// this file is about what `cash.ts` writes, and a reconciliation test that also
// exercised another package's Dnevnik rendering would fail for two reasons at
// once. `changes-coverage.test.ts` is where the real ones run end to end.
