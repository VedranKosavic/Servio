...
/**
 * The routes whose files exist today: Korak 1's, plus WP1's own row of the
 * work-package table. Everything else in `ROUTE_ROLES` belongs to a package that
 * has not merged — `POST /api/devices/heartbeat` is WP5's and `/api/me/shift*`
 * are WP2's, which is why this is an explicit list and not a prefix match.
 *
