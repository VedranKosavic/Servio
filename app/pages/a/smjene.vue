 ...
 * is one poll on `/a` and this page subscribes to it rather than opening a timer
 * of its own.
 */
    ...
    <UiCard title="Smjene" :count="rows.length">
      ...
      <UiTable v-else :columns="columns" :loading="loading">
        <tr v-for="row in rows" :key="row.id" class="a-row">
          <td>
            <NuxtLink :to="`/a/smjena/${row.id}`">{{ dateBs(row.business_date) }}</NuxtLink>
          </td>
          <td>
          ...
          <td class="r">
            <NuxtLink :to="`/a/smjena/${row.id}`" aria-label="Otvori smjenu">
