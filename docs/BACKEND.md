...
**The seed needs `PIN_PEPPER`, and now says so.** `npm run db:seed` runs `seed-cli.ts` through `tsx`, outside Nuxt — so nothing loads `.env` for it, and `peppered()` used to read `process.env.PIN_PEPPER ?? ''`. A missing pepper is not "no pepper", it is a *different* pepper: the seed wrote hashes that no server started with `PIN_PEPPER=dev` could verify, every PIN in the fresh database was wrong, and the only symptom was a login screen refusing everybody until the device locked. Two things fix it, and both are in place: the script is `node --env-file-if-exists=.env --import tsx …` (the shell still wins, so `DB_PATH=data/verify.db npm run db:seed` still seeds `data/verify.db`), and hashing with no pepper throws outright — except under vitest, where `vitest.config.ts` supplies the fixture pepper. A production seed (`NODE_ENV=production`) hashes nothing and is unaffected.
...
**Menu-article otpis (0008).** The owner values an otpis at the **menu price**, and staff pick what was spilled from the menu. A body with `product_id` (and `flavour_ids` for a nargila, validated by `requireFlavours` exactly as on an order line) writes one `product_waste` row: `unit_price_fen = products.price_fen` snapshotted now, `value_fen = round(unit_price_fen × qty)`, and one `waste` movement per stock item `resolveStock()` returns for that product and quantity (`ref_type='product_waste'`, each at its own purchase cost) — none for a product with no stock link. `waste_events` is not written, because its `stock_item_id` is NOT NULL and relaxing it is a table rebuild. The same gates apply, with the threshold read against `value_fen`; the bottle rule looks at the product's 1:1 item; the per-shift cap counts both tables; the entry is `log('product_waste_logged')`. Replay by `client_id` looks in both tables first. `approveWaste` finds the id in either table. The `stock_item_id` shape stays accepted for entries still queued in a phone's outbox, and old `waste_events` rows stay readable at their purchase-cost value. `waste_fen` / `waste { count, fen }` on the summaries add both tables. The *Kategorije* report's *Otpis* column is still movements at cost, grouped by stock category, and includes count adjustments.
...
| `POST /api/stock/waste` | A W B | `{ client_id, product_id? \| stock_item_id? (exactly one), flavour_ids?, qty >0, reason, note?, approver_user_id?, pin?, client_created_at? }` | `WasteView { id, product_id, stock_item_id, needs_approval, cost_fen, unit_price_fen, on_hand, already_applied }` | 403 `REASON_FORBIDDEN`; 404 `PRODUCT_NOT_FOUND` / `STOCK_ITEM_NOT_FOUND`; 400 `FLAVOURS_REQUIRED` |
...
| `product_waste_logged` | Otpis · {user} · {product} {qty} · {value} · {reason} | waste_id | `logWaste` (menu article, 0008) | quiet unless `needs_approval` | |
