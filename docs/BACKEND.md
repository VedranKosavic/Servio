...
| `pin_len` | `INTEGER NOT NULL DEFAULT 4` | 4 or 6; Zod enforces, no trigger |
| `pin_set_at` | `TEXT` | |
...
`GET /api/auth/users` returns `MeUser[]` with `has_pin` and nothing else — it is served **before** any session exists, so it is the one response a stranger holding an enrolled device can read, and it must contain no more than the lock screen draws.
...
| `POST /api/devices/enrol` | public | `{ code: z.string().length(6), label?: ≤40, app_version?: ≤20 }` | `{ device, venue, users: MeUser[] }` + `sank_d` | 400 `ENROL_CODE_INVALID` |
...
| `GET /api/auth/users` | public (needs `sank_d`) | — | `MeUser[]` (active staff of the device's venue) | 401 `NO_DEVICE` |
...
| `POST /api/dev/enrol` | public, `SANK_DEV_ENROL=1` | `{}` | `{ device, users }` + `sank_d` | 404 `NOT_FOUND` in every other environment |
