...
/**
 * The belt to `ROUTE_ROLES`' braces (`docs/BACKEND.md` §2).
 *
 * `server/middleware/tenant.ts` is the real gate and it denies by default, so a
 * handler that reaches this line has already been let through. It is said again
 * in the handler because the two owner-only reads — the Dnevnik and its
 * drill-down — are a product rule and not a routing detail: a route renamed or
 * re-declared in `ROUTE_ROLES` by mistake must not quietly open them.
 */
