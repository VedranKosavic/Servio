 ...
 *
 * **The server owns the first three.** *Sav prihod* is the shift's promet and
 * *Otpis* its waste value, both read out of `summarizeShift` — the same call that
 * writes `shift_summaries`, so the closing, the summary and *Smjena* cannot print
 * three different nights. *Dnevnica* is `settings.dnevnica_fen`, once per shift.
 * The phone sends only the five amounts nobody but the šanker can know.
 *
...
/** The five typed amounts, as `closeByBarBody` has already parsed them. */
...
/**
 * The server's three numbers for a shift, right now.
 *
 * `summarizeShift` is the one definition of promet (charged − applied voids)
 * and of the shift's otpis (`product_waste.value_fen` + legacy
...
): { prihod_fen: number, dnevnica_fen: number, otpis_fen: number } {
