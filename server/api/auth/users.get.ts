 ...
 * initials, role, PIN length, and whether a PIN is set at all. No email, no
 * hash, nothing that would make the list worth stealing (§5.5).
 */
...
export default defineEventHandler(event => guard(() => {
  ...
  return listLoginUsers(db, device.venueId)
}))
